...
def retrieval_metrics(documents, expected_topics):

    docs = [
        normalize(document_text(doc))
    ...
    for doc in docs:
        ...
        if any(topic in doc for topic in topics):
            relevant_docs += 1
...
def topic_coverage(answer, topics):
...
def context_support(answer, documents):
...
def main():
    ...
    print("=" * 70)
    print("COSMORA - SKIN RAG EVALUATION")
    print("=" * 70)
    ...
    for case_number, case in enumerate(dataset, start=1):
        ...
        print(
            f"Hit Rate@5  : {retrieval['hit_rate_at_5']:.3f}"
        )
    ...
    print(
        f"Answer Correctness : {avg_correctness:.3f}"
    )
